    ...
    with ZipFile(file_name, "r") as zip_file:
        for z in zip_file.infolist():
            if not config.file_extensions or (
                ...
                (_, basename) = os.path.split(z.filename)
                (_, extension) = os.path.splitext(basename)
                contents = zip_file.read(z).decode(errors="ignore")
                if not contents:
                    contents = "EMPTY FILE"
                elif "\0" in contents:
                    contents = "BINARY"
