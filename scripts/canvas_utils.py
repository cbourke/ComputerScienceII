"""
This module provides a collection of functions to interface
with the Canvas API.
"""
...
def get_canvas_data(path, user_parameters=None):
    ...
    while more_data:
        ...
        if "next" in links:
            ...
            #path = links['next'].removeprefix("https://canvas.unl.edu") + f"&{params}"
        else:
            more_data = False
    connection.close()
    return data
...
def get_sections():
    """
    Retrieves all sections from the canvas course.
    """
...
def set_grade(assignment_id, user_id, score=None, comment=None):
    ...
    url = f"https://{CANVAS_HOST}{CANVAS_BASE_API_PATH}/courses/{config.canvas_course_id}/assignments/{assignment_id}/submissions/{user_id}"
...
def get_assignment_group_d(assignment_group_name):
    ...
    groups = { group_id for group_id, name in groups.items() if name == assignment_group_name }
